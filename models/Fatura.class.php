...
class Fatura{
    ...
    function addFatura($num, $lancada, $venc, $cat, $fornec, $pagar, $status){
        if($this->con->exec("INSERT INTO faturas (num,lancada,vencimento,categoria,fornecedor,pagar, status) VALUES ('{$num}', '{$lancada}', '{$venc}', '{$cat}', '{$fornec}', '{$pagar}', '{$status}')")){
    ...
    function editeFatura($id, $lancada, $venc, $cat, $fornec, $pagar, $desc, $pago, $status){
        if($this->con->exec("UPDATE faturas SET lancada = '{$lancada}', vencimento = '{$venc}', categoria = '{$cat}', fornecedor = '{$fornec}', pagar = '{$pagar}', desconto = '{$desc}', pago = '{$pago}', status = '{$status}' WHERE id = '{$id}'")){
